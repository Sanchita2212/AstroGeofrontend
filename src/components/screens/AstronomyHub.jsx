...
function LaunchesTab() {
  ...
  const nextIsroLaunch = useMemo(() => {
    ...
    return {
      ...
      window_start: new Date(Date.now() + 23 * 24 * 60 * 60000 + 14 * 60 * 60000 + 32 * 60000).toISOString(),
      status: { name: 'TBD' },
      mission: { name: 'ESA Solar Mission' },
    }
  }, [launches])

  const countdown = formatCountdown(nextIsroLaunch.window_start)
        ...
        <GlassPanel className="p-8 relative overflow-hidden bg-gradient-to-b from-[#111827]/80 to-[#1e1b4b]/40">
          ...
          <div className="bg-indigo-900/30 border border-indigo-500/30 rounded-xl p-5 mb-6">
            <div className="flex justify-between items-center mb-4">
              <div className="text-indigo-400 font-bold flex items-center gap-2">🤖 AI PREDICTION (On-Time)</div>
            ...
            <div className="space-y-3 text-sm text-slate-300">
              ...
              <div className="text-xs text-slate-500 pt-2 border-t border-indigo-500/20">Similar past launches: 28/32 on-time (87.5%)</div>
            </div>
            <div className="mt-4 flex gap-3">
              <button className="flex-1 bg-slate-800 hover:bg-slate-700 text-xs font-semibold py-2 rounded-lg transition-colors border border-slate-600">✅ Verify Prediction</button>
            </div>
          </div>
